...
public class TokenSecurityFilter implements Filter {
		...
		if ("login".equals(path) || "usuarios".equals(path)) {
			...
			try {
				...
				if (e.getClaims().containsKey("content")) {
					Usuario user = this.getManagerSecurity().getContentJWT(e.getClaims().get("content").toString());
					System.out.println(
	 ...
	 */
	public ServicioGuarani getLoginService() {
		return loginService;
	}
	 ...
	 */
	public void setLoginService(ServicioGuarani loginService) {
